...
impl Board {
    ...
    pub fn set_value(&mut self, index: usize, value: u32) -> bool {
        ...
        if r == c {
            for i in 0..9 {
                let cell_index = i * 9 + r;
                if cell_index != index {
                    if !self.clear_value(i * 9 + i, value_mask) {
                        return false;
                    }
                }
            }
        ...
        if r == 8 - c {
            for i in 0..9 {
                let cell_index = i * 9 + r;
                if cell_index != index {
                    if !self.clear_value(i * 9 + (8 - i), value_mask) {
                        return false;
                    }
                }
            }
...
const HOUSES: [[usize; 9]; 29] = [
    ...
    [0 * 9 + 0, 1 * 9 + 1, 2 * 9 + 2, 3 * 9 + 3, 4 * 9 + 4, 5 * 9 + 5, 6 * 9 + 6, 7 * 9 + 7, 8 * 9 + 8],
    [0 * 9 + 8, 1 * 9 + 7, 2 * 9 + 6, 3 * 9 + 5, 4 * 9 + 4, 5 * 9 + 3, 6 * 9 + 2, 7 * 9 + 1, 8 * 9 + 0],
];
